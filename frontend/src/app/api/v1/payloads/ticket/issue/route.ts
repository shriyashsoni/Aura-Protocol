...
export async function POST(req: NextRequest) {
    ...
    if (!parsed.success) {
    return NextResponse.json({ error: "Invalid issue ticket payload", issues: parsed.error.issues }, { status: 400 });
    }
    
    return NextResponse.json(template(config.programs.accessTicketing, "issue_ticket", parsed.data));
    } catch {
